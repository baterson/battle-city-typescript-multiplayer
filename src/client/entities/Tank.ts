...
export class Tank extends Movable {
  ...
  /*
	...
	*/
  shot(cd: number, game: HeadlessGame) {
    ...
    if (this.direction === Direction.Top) {
      bulletPosition = {
        x: this.size.x / 2 + this.position.x - width / 2,
        y: this.position.y - height,
      };
    } else if (this.direction === Direction.Right) {
      bulletPosition = {
        x: this.size.x + this.position.x,
        y: this.size.y / 2 + this.position.y - height / 2,
      };
    ...
    game.entityManager.spawnEntity(
      "Bullet",
      bulletPosition,
      this.direction,
      this
