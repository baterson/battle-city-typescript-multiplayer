...
export class Enemy extends Tank {
  ...
  render() {
    ...
    if (this.type === TankTypes.Armored) {
      this.animateMovement(
        assetsHolder.sprites.enemy[this.type][this.lives][this.direction]
      );
    } else {
      this.animateMovement(
        assetsHolder.sprites.enemy[this.type][this.direction]
      );
    }
  }
  ...
  /*
	...
	*/
  isSpawnSpotClear(game: HeadlessGame) {
    return game.entityManager.entities
      ...
      .some((entity) =>
        checkEntityCollision(entity.getBoundingBox(), this.getBoundingBox())
      );
  }
